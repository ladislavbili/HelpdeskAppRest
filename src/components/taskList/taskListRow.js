  ...
  render() {
    return (
      <ListItem button onPress={()=>{this.props.startLoading();Actions.taskEdit({id:this.props.task.id})}} >
              <Body>
                <Text>{this.props.task.title?this.props.task.title:'None'}</Text>
                ...
                <Text numberOfLines={1} note>{I18n.t('deadline')}: {this.props.task.deadline?this.props.task.deadline:I18n.t('noDate')}</Text>
